...
pub fn generate_dashboard() {
    ...
    display_html_dashboard(dashboard);
}

fn display_html_dashboard(dashboard: dashboard::Tree) {
    // one template only
    let tera = Tera::new("src/tools/dashboard/src/templates/*.html").unwrap();
    let mut context = Context::new();
    context.insert("dashboard", &dashboard);
    let mut q: Vec<dashboard::Tree> = Vec::new();
    context.insert("q", &mut q);
    let out = tera.render("dashboard.html", &context).unwrap();
    fs::write("src/tools/dashboard/index.html", &out);
}
